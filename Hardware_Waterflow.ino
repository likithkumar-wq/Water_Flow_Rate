...
// --- Simulated RTC time speedup ---
DateTime baseTime;
unsigned long baseMillis;
float speedFactor = 1.0; // 1/0.75: "seconds" simulate every 750ms (33% faster)
...
void setup() {
  ...
  if (rtc.lostPower()) {
    // rtc.adjust(DateTime(F(__DATE__), F(__TIME__))); // Optional: Commented to allow serial time set
  }
...
void loop() {
  ...
  // --- Simulate faster time ---
  unsigned long elapsed = currentTime - baseMillis;
  unsigned long adjustedElapsed = (unsigned long)(elapsed * speedFactor);
  DateTime simTime = baseTime + TimeSpan(adjustedElapsed / 1000);
  ...
  if (currentTime - lastUpdateTime >= 1000) {
    ...
    // OLED Display using simTime
    display.clearDisplay();
    display.setCursor(0, 0);
    if (simTime.hour() < 10) display.print('0');
    display.print(simTime.hour());
    display.print(':');
    if (simTime.minute() < 10) display.print('0');
    display.print(simTime.minute());
    display.print(':');
    if (simTime.second() < 10) display.print('0');
    display.print(simTime.second());
  ...
  // Bluetooth data every 1 minute (display simTime)
  if (currentTime - lastBluetoothTransmit >= 60000 && samplesCollected > 0) {
    float avgFlowRate = sumFlowRate / float(samplesCollected);
    BTSerial.print("Time: ");
    if (simTime.hour() < 10) BTSerial.print('0');
    BTSerial.print(simTime.hour()); BTSerial.print(':');
    if (simTime.minute() < 10) BTSerial.print('0');
    BTSerial.print(simTime.minute());
    ...
    Serial.print("Time: ");
    if (simTime.hour() < 10) Serial.print('0');
    Serial.print(simTime.hour()); Serial.print(':');
    if (simTime.minute() < 10) Serial.print('0');
    Serial.print(simTime.minute());
